class UsersController < ApplicationController
  ...
  def show
    ...
    @edit_at_count = Attendance.joins(:user).where(edit_app_n: current_user.name).where.not(edit_finish: nil).where.not(user_id: current_user.id).where(edit_change_b: "0").count
  end
